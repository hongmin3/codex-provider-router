...
- Windows에서 보존한 원본 Codex 경로가 사라지거나 Router 자신의 shim을 가리키면, 자신의
  shim을 제외한 다음 `codex` 실행기를 PATH에서 찾아 경로를 갱신하고 실행을 계속한다. 대체
...
`test_router_keeps_the_missing_path_error_when_no_alternative_exists`를 실행한다.
...
- **비용 계산의 커버리지.** 2026-09-19 이전 세션은 원장에 없어 한도 계산에 포함되지
  ...
  작을 수 있다.
- **macOS 원본 Codex 경로.** 원본 실행 파일 경로가 Apple Silicon Homebrew 기준 한 값으로
